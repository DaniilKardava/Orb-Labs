...
contract Vault is VaultBase {
    ...
    function completeVaultWithdrawals() public override {
        while ((eigenWithdrawalQueue.length() > 0) && checkEigenWithdrawal()) {
    ...
    function queueEigenWithdraw(uint256 assets) internal override {
        ...
        // M2 withdrawal struct
        IDelegationManager.Withdrawal
            memory queuedWithdrawal = IDelegationManager.Withdrawal({
                staker: address(this),
                delegatedTo: eigenContracts.delegationManagerProxy.delegatedTo(
                    address(this)
                ),
                withdrawer: address(this),
                nonce: eigenContracts
        ...
        // M2 Queue Withdrawal Params object
        ...
        bytes32[] memory roots = eigenContracts
            .delegationManagerProxy
            .queueWithdrawals(queuedWithdrawalParams);

        eigenWithdrawalQueue.enqueue(roots[0], queuedWithdrawal);
    }
    ...
    function completeEigenWithdrawal() internal override {
        ...
        eigenContracts.delegationManagerProxy.completeQueuedWithdrawal(
    ...
    function servePriorityQueue() internal override {
        while (
            (vaultPriorityWithdrawalQueue.length() > 0) &&
